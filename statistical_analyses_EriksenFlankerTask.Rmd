...
```{r}
Flanker_data$Light <- relevel(Flanker_data$Light, ref = "Dim")
```
...
*Fit linear mixed-effects model, check model assumptions and performance metrics* 

```{r}
set.seed(123)

IIVRT_incompatible_mod <- lmer(EFT_sdRTincompatible ~ EFT_RTincompatible + Light + sdTST + (1 | Subject),
                                     data = Flanker_data
                                     )
```

```{r}
performance::check_model(IIVRT_incompatible_mod)

plot(DHARMa::simulateResiduals(IIVRT_incompatible_mod))
```

```{r}
performance::model_performance(IIVRT_incompatible_mod)
```
...
```{r}
summary(IIVRT_incompatible_mod)
```

```{r}
performance::model_parameters(IIVRT_incompatible_mod)
```


*Calculate approximated effect sizes for all fixed effects irrespective of significance, based on test statistics (output from summary() above)*
...
```{r}
emmeans_IIVRTincompatible_light <- emmeans(IIVRT_incompatible_mod, specs = ~ Light)
emmeans_IIVRTincompatible_light_df <- summary(as.data.frame(IIVRT_incompatible_mod))
...
conflict_effect_mod <- lmer(ExecutiveNetwork_score ~ Light + sdTST + (1 | Subject),
                                      data = Flanker_data
                                      )
```
...
```{r}
performance::model_parameters(conflict_effect_mod)
```
...
emmeans_conflict_effect_light_df <- summary(as.data.frame(conflict_effect_mod))
...
IIVRT_compatible_mod <- lmer(EFT_sdRTcompatible ~ EFT_RTcompatible + Light + sdTST + (1 | Subject),
                                     data = Flanker_data
                                     )
```

```{r}
performance::check_model(IIVRT_compatible_mod)

plot(DHARMa::simulateResiduals(IIVRT_compatible_mod))
```

```{r}
performance::model_performance(IIVRT_compatible_mod)
```
...
```{r}
summary(IIVRT_compatible_mod)
```

```{r}
performance::model_parameters(IIVRT_compatible_mod)
```
...
```{r}
emmeans_IIVRTcompatible_light <- emmeans(IIVRT_compatible_mod, specs = ~ Light)
emmeans_IIVRTcompatible_light_df <- summary(as.data.frame(IIVRT_compatible_mod))
...
RT_incompatible_mod <- lmer(EFT_RTincompatible ~ Light + sdTST + (1 | Subject),
                                     data = Flanker_data
                                     )
```
...
```{r}
performance::model_parameters(RT_incompatible_mod)
```
...
RT_compatible_mod <- lmer(EFT_RTcongruent ~ Light + sdTST + (1 | Subject),
                                     data = Flanker_data
                                     )
```
...
```{r}
performance::model_parameters(RT_compatible_mod)
```
...
*Add a small constant to accuracy values equal to 1 so that the data can be modeled using beta regression*

```{r}
Flanker_data_accuracy <- Flanker_data_accuracy %>% 
  mutate(
    EFT_Accuracycompatible = ifelse(EFT_Accuracycompatible == 1, 1 - 0.00001, EFT_Accuracycompatible),
    EFT_Accuracyincompatible = ifelse(EFT_Accuracyincompatible == 1, 1 - 0.00001, EFT_Accuracyincompatible)
    )
```

*Re-level categorical predictor*

```{r}
Flanker_data_accuracy$Light <- relevel(Flanker_data_accuracy$Light, ref = "Dim") #re-level light so that dim is the reference level
```

### Incompatible task condition

*Fit generalized linear mixed-effects model assuming with a beta distribution and logit link function, check model assumptions and performance metrics*

```{r}
set.seed(123)

Accuracyincompatible_mod <- glmmTMB(EFT_Accuracyincompatible ~ Light + sdTST + (1 | Subject),
                                             data = Flanker_data_accuracy,
                                             family = beta_family(link = "logit")
                                             )
```

```{r}
performance::check_model(Accuracyincompatible_mod)

plot(DHARMa::simulateResiduals(Accuracyincompatible_mod))
```

```{r}
performance::model_performance(Accuracyincompatible_mod)
```
...
```{r}
summary(Accuracyincompatible_mod)
```

```{r}
performance::model_parameters(Accuracyincompatible_mod)
```
...
Accuracycompatible_mod <- glmmTMB(EFT_Accuracycompatible ~ Light + sdTST + (1 | Subject),
                                             data = Flanker_data_accuracy,
                                             family = beta_family(link = "logit")
                                             )
```
...
```{r}
performance::check_model(Accuracycompatible_mod)

plot(DHARMa::simulateResiduals(Accuracycompatible_mod))
```

```{r}
performance::model_performance(Accuracycompatible_mod)
```
...
```{r}
summary(Accuracycompatible_mod)
```

```{r}
performance::model_parameters(Accuracycompatible_mod)
```
