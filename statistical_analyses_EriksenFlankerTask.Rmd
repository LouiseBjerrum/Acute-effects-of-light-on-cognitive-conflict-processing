...
# Session info and loaded package versions 
...
*compute Pearson's correlation between intra-individual variability in RT and mean RT*

```{r}
Flanker_data_Pearsons <- Flanker_data %>% 
  select(IIV_RT_incompatible, RT_incompatible) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Pearsons,
         "IIV_RT_incompatible", "RT_incompatible",
         method = "pearson")
```

*Compute Spearman's rank correlation between intra-individual variability in RT and intra-individual variability in TST*

```{r}
Flanker_data_Spearmans <- Flanker_data %>% 
  select(IIV_RT_incompatible, IIV_TST) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Spearmans,
         "IIV_RT_incompatible","IIV_TST",
         method = "spearman")
```
...
```{r}
Flanker_data_Spearmans <- Flanker_data %>% 
  select(Conflict_effect_score, IIV_TST) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Spearmans,
         "Conflict_effect_score","IIV_TST",
         method = "spearman")
```
...
```{r}
Flanker_data_Pearsons <- Flanker_data %>% 
  select(IIV_RT_compatible, RT_compatible) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Pearsons,
         "IIV_RT_compatible", "RT_compatible",
         method = "pearson")
```
...
```{r}
Flanker_data_Spearmans <- Flanker_data %>% 
  select(IIV_RT_compatible, IIV_TST) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Spearmans,
         "IIV_RT_compatible","IIV_TST",
         method = "spearman")
```
...
*Compute Spearman's rank correlation between mean RT and intra-individual variability in TST*

```{r}
Flanker_data_Spearmans <- Flanker_data %>% 
  select(RT_incompatible, IIV_TST) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Spearmans,
         "RT_incompatible","IIV_TST",
         method = "spearman")
```
...
```{r}
Flanker_data_Spearmans <- Flanker_data %>% 
  select(RT_compatible, IIV_TST) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Spearmans,
         "RT_compatible","IIV_TST",
         method = "spearman")
```
...
*Compute Spearman's rank correlation between mean accuracy and intra-individual variability in TST*

```{r}
Flanker_data_Spearmans <- Flanker_data %>% 
  select(Accuracy_incompatible, IIV_TST) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Spearmans,
         "Accuracy_incompatible","IIV_TST",
         method = "spearman")
```
...
```{r}
Flanker_data_Spearmans <- Flanker_data %>% 
  select(Accuracy_compatible, IIV_TST) %>% 
  na.omit() 
```

```{r}
cor_test(Flanker_data_Spearmans,
         "Accuracy_compatible","IIV_TST",
         method = "spearman")
```
